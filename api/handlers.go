...
func (h *Handlers) SaveNewMission(w http.ResponseWriter, r *http.Request) {
	...
	var missionID int
	missionSQL := `
		INSERT INTO missions (user_id, uav_id, mission_name, schedule, is_recurring, status) 
...
func (h *Handlers) GetAllMissions(w http.ResponseWriter, r *http.Request) {
	rows, err := h.DB.Query(`SELECT id, user_id, mission_name, is_recurring, status, timestamp FROM missions ORDER BY timestamp DESC`)
	if err != nil {
...
func (h *Handlers) GetLastMission(w http.ResponseWriter, r *http.Request) {
    ...
    err = row.Scan(&mission.ID, &mission.UserID, &mission.MissionName, &mission.IsRecurring, &mission.Status, &mission.Timestamp)
